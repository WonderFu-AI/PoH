...
import { listSessions, getSessionMessages, searchSessions, deleteSession, getSessionMetadata } from "./sessions";
import {
  syncSessionCache,
  listCachedSessions,
  updateSessionTitle,
...
function setupIPC(): void {
  ...
  ipcMain.handle("set-locale", (_event, locale: "en" | "zh-CN") => setAppLocale(locale));
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (attachmentPath) {
        console.log("[PoH] attachmentPath received:", attachmentPath);
      }
      // Lazy start: ensure gateway is running on first chat
      if (!isGatewayRunning()) {
        startGateway(profile);
        // Wait for API server to be ready before sending message
  ...
  ipcMain.handle("get-gateway-state", (): ReturnType<typeof readGatewayState> =>
    readGatewayState(),
  );
  ...
  ipcMain.handle("get-session-metadata", (_event, sessionId: string) => {
  ...
  ipcMain.handle(
    "add-model",
    (_event, name: string, provider: string, model: string, baseUrl: string, apiKey: string) =>
      addModel(name, provider, model, baseUrl, apiKey),
  );
  ipcMain.handle("remove-model", (_event, id: string) => removeModel(id));
  ipcMain.handle(
    "update-model",
    (_event, id: string, fields: Record<string, string>) =>
      updateModel(id, fields as Parameters<typeof updateModel>[1]),
  );
...
app.on("before-quit", (event) => {
  // If an external gateway is running (started by user), ask before killing all
  if (getGatewayOwner() === "external") {
    event.preventDefault();
    dialog.showMessageBox({
      type: "question",
      buttons: ["仅关闭 PoH", "全部关闭"],
      defaultId: 0,
      cancelId: 0,
      title: "关闭 Hermes Gateway",
      message: "检测到你通过终端启动了 Hermes Gateway",
      detail: "是否同时关闭 Hermes Gateway？选择「仅关闭 PoH」将保留 Hermes Gateway 运行。",
    }).then(({ response }) => {
      stopHealthPolling();
      stopGateway(true); // force stop PoH's gateway if any
      stopClaw3d();
      if (response === 1) {
        killExternalGateway();
      }
