...
const hermesAPI = {
  ...
  getSessionMetadata: (sessionId: string): Promise<{ startedAt: number } | null> =>
    ipcRenderer.invoke("get-session-metadata", sessionId),
  ...
  installDocsSkill: (): Promise<{ success: boolean }> =>
    ipcRenderer.invoke("install-docs-skill"),
  getDocsSkillContent: (): Promise<string> =>
    ipcRenderer.invoke("get-docs-skill-content"),
  ...
  addModel: (
    ...
    apiKey: string;
    createdAt: number;
  }> => ipcRenderer.invoke("add-model", name, provider, model, baseUrl, apiKey),
